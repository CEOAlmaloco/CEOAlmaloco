"""
Build a neofetch-style info card SVG for Alex Ampuero (CEOAlmaloco).
Adapted from AVIVASHISHTA29/scripts/make_info_card.py
"""
import html
import os
...
ROWS = [
    ("host",),
    ("kv", "Role", "Full Stack / Backend Developer"),
    ("kv", "Focus", "APIs, Cloud, DevOps & Mobile"),
    ("kv", "Loc", "Valparaiso, Chile"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Backend", "Java, Spring Boot, Node.js, JWT/OAuth"),
    ("kv", "Frontend", "React, TypeScript, Kotlin/Android"),
    ("kv", "Data", "PostgreSQL, MySQL, MongoDB, Oracle"),
    ("kv", "Cloud", "AWS, Azure, Docker, GitHub Actions"),
    ("kv", "QA", "SonarQube, Postman, TDD, Linters"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Curious builder — iterate until it feels right"),
    ("bul", "Copilot · Cursor · Claude Code"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">alex@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">alex</tspan><tspan fill="{MUTED}">@</tspan>'
